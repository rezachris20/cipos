...
class Page extends CI_Controller
{
	...
	function index()
	{
		...
		$data['penjualan'] = $this->db->query("SELECT sum(tb_penjualan.total)as totalpenjualan,sum(tb_penjualan.profit_penjualan)as totalprofit FROM tb_penjualan WHERE tgl_penjualan ='$date'")->result();
		...
		$data['penjualan1'] = $this->db->query("SELECT tb_penjualan.kode_penjualan,tb_barang.kode_barcode, tb_penjualan.tgl_penjualan,tb_barang.nama_barang,sum(tb_penjualan.jumlah) as totjum,sum(tb_penjualan.subtotal) as subtotal,sum(tb_penjualan.diskon) as totdiskon, sum(tb_penjualan.total) as totjual,sum(tb_penjualan.profit_penjualan) as totprofit from tb_barang, tb_penjualan where tb_barang.kode_barcode = tb_penjualan.kode_barcode and status='1' group by tgl_penjualan DESC LIMIT 30")->result();
