...
class Pending extends CI_Controller
{
	...
	{
		$data['laporan'] = $this->db->query ("SELECT id_penjualan, tb_penjualan.tgl_penjualan,tb_penjualan.kode_penjualan, tb_barang.nama_barang,tb_penjualan.jumlah,sum(tb_penjualan.jumlah)as jumlah,sum(tb_penjualan.subtotal)as subtotal,sum(tb_penjualan.diskon)as diskon,sum(tb_penjualan.total)as total FROM tb_penjualan, tb_barang WHERE tb_penjualan.kode_barcode = tb_barang.kode_barcode AND status = '0' GROUP BY tb_penjualan.kode_penjualan ")->result();
